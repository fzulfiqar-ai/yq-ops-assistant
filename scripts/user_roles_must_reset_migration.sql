...
-- looked at it — 14 field logins are still on the temp password. The API now enforces
-- user_roles.must_reset (app/auth.py: 403 'password change required' on every route except
-- /me, /auth/features and POST /auth/password) and clears it only after it set the new password
-- itself (app/user_auth.set_password).
--
-- Deploy order: the API tolerates the column being absent (treated as false), so deploy the API
-- FIRST, then apply this file. Nothing here touches views or grants; user_roles stays
-- service-role only (hotfix_view_grants_migration.sql revoked anon/authenticated).
...
-- temporary password are asked to change it. Only ever sets TRUE where the metadata says so and
-- the row is still false; a member who already changed their password (the SPA wrote
-- must_reset=false) is left alone. Re-running is a no-op. Skipped when auth.users is not
-- readable from this role (a local scratch database).
do $$
begin
...
-- Self-check: the column exists with the right shape and the table is still not reachable by the
-- browser roles (neither table nor column grants).
do $$
declare
  col record;
begin
