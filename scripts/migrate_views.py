...
Views (LLM queries ONLY these -- never raw tables):
  v_sales           enriched sales lines: order header + product + category
  v_current_stock   latest balance per item+warehouse (MAX-id rule 6)
  v_product_margin  Focus COGS basis margin snapshot (rule 1)
  v_receivables     latest outstanding balance per account
  v_top_customers   revenue ranking per customer
  v_sales_by_period monthly trend
  v_low_stock       items with balance_qty <= 10 (refs v_current_stock)

Also creates yq_readonly role (SELECT-only on the 7 views + shipments).
"""
...
SQL_OUT = ROOT / "scripts" / "views.sql"
