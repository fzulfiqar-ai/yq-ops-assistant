...
/** FastAPI's {"detail": "reason"} → the reason; anything else → the fallback. */
function detailOf(e: ApiError, fallback: string): string {
  try {
 ...
 *
 * `currentPassword` is required by the API unless the login is still on its temporary password
 * (must_reset): a stolen access token alone must not be enough to take the account over.
 * Resolves to an error message to show, or null on success.
 */
export async function changeOwnPassword(password: string, currentPassword?: string): Promise<string | null> {
  try {
    await apiPost('/auth/password', { password, current_password: currentPassword || null })
  } catch (e) {
    ...
    if (e instanceof ApiError) {
      if (e.status === 400) return detailOf(e, 'Password must be 8 to 128 characters.')
      if (e.status === 403) return detailOf(e, 'Current password is incorrect.')
